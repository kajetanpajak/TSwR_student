...
traj_gen = Sinusoidal(np.array([0.0, np.pi/2]), np.array([2.0, 2.0]), np.array([0., 0.]))
...
plt.plot(T, Q_d[:, 0], 'b', label=r'$q_{1d}(t)$')
plt.legend()
plt.subplot(222)
plt.plot(T, Q[:, 1], 'r', label=r'$q_2(t)$')
plt.plot(T, Q_d[:, 1], 'b', label=r'$q_{2d}(t)$')
plt.legend()
plt.subplot(223)
plt.plot(T, u[:, 0], 'r', label=r'$\tau_1(t)$')
plt.plot(T, u[:, 1], 'b', label=r'$\tau_2(t)$')
plt.legend()
plt.show()
